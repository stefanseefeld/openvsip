...
public:
  ...
  void query_layout(dda::Rt_layout<1> &rtl_inout)
  {
    rtl_inout.pack = dda::stride_unit_dense;
    rtl_inout.complex = dda::cmplx_inter_fmt;
  }

  // Similarly, for out-of-place data, this backend requires unit
  // strides and complex interleaved format in both the input and
  // output data.
  void query_layout(dda::Rt_layout<1> &rtl_in, dda::Rt_layout<1> &rtl_out)
  {
    rtl_in.pack = rtl_out.pack = dda::stride_unit_dense;
    rtl_in.complex = rtl_out.complex = dda::cmplx_inter_fmt;
  }
};
