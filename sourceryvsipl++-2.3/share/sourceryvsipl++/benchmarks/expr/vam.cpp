...
*/
/** @file    benchmarks/expr/vam.cpp
    @author  Mike LeBlanc
    @date    2009-10-08
    @brief   VSIPL++ Library: Benchmark for fused add-multiply
...
struct t_vam : Benchmark_base
{
  ...
  void diag()
  {
    ...
    impl::assign_diagnostics(X, (A.view + B.view) * C.view);
  }
};
...
//    X = (A.view + B.view) * C.view;
  ...
  void diag()
  {
    ...
    impl::assign_diagnostics(X, am(A.view,B.view,C.view));
  }
};
...
struct t_vam_nonfused : Benchmark_base
{
  ...
  void diag()
  {
    ...
    impl::assign_diagnostics(tmp, A.view + B.view);
    impl::assign_diagnostics(X,  tmp * C.view);
  }
};
...
struct t_vam_ip : Benchmark_base
{
  ...
  void diag()
  {
    ...
    impl::assign_diagnostics(X, X * (A.view + B.view));
  }
};
...
struct t_vam_cSC : Benchmark_base
{
  ...
  void diag()
  {
    ...
    impl::assign_diagnostics(X, (a + B) * C);
  }
};
...
struct t_vam_cSC_simd : Benchmark_base
{
    ...
    VSIP_IMPL_NOINLINE
  {
    ...
    t1.start();
    for (index_type l=0; l<loop; ++l)
      vsip::impl::simd::vam_cSC(a, B_ext.data(), C_ext.data(), X_ext.data(),
				size);
    t1.stop();
    }
