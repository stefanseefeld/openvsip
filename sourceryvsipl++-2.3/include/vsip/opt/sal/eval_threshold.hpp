...
struct Threshold_evaluator
{
		   ...
		   expr::Scalar<1, T> const &c)
  {
    ...
    sal::Ext_wrapper<LHS, dst_lp>  ext_dst(lhs, SYNC_OUT);
    sal::Ext_wrapper<B, block1_lp> ext_A(a, SYNC_IN);
    sal::Ext_wrapper<sb_type> ext_b(b, SYNC_IN);

    if (c.value() == T(0))
      sal::vthresh0(typename sal::Ext_wrapper<B, block1_lp>::sal_type(ext_A),
		    typename sal::Ext_wrapper<sb_type>::sal_type(ext_b),
		    typename sal::Ext_wrapper<LHS, dst_lp>::sal_type(ext_dst),
		    lhs.size());
    else
      sal::vthresh(typename sal::Ext_wrapper<B, block1_lp>::sal_type(ext_A),
		   typename sal::Ext_wrapper<sb_type>::sal_type(ext_b),
		   typename sal::Ext_wrapper<LHS, dst_lp>::sal_type(ext_dst),
		   lhs.size());
  }
...
{									\
  ...
  {									\
    ...
    typedef expr::Scalar<1, T> sb_type;				        \
									\
    sal::Ext_wrapper<LHS, dst_lp>  ext_dst(lhs, SYNC_OUT);	        \
    sal::Ext_wrapper<B, block1_lp> ext_A(rhs.arg2(), SYNC_IN);	        \
    sal::Ext_wrapper<sb_type> ext_b(rhs.arg1().arg2(), SYNC_IN);        \
									\
    if (rhs.arg3().value() == T(0))					\
      FUN0(typename sal::Ext_wrapper<B, block1_lp>::sal_type(ext_A),    \
           typename sal::Ext_wrapper<sb_type>::sal_type(ext_b),		\
	   typename sal::Ext_wrapper<LHS, dst_lp>::sal_type(ext_dst),   \
	   lhs.size());							\
    else								\
      FUN(typename sal::Ext_wrapper<B, block1_lp>::sal_type(ext_A),     \
	  typename sal::Ext_wrapper<sb_type>::sal_type(ext_b),		\
	  typename sal::Ext_wrapper<LHS, dst_lp>::sal_type(ext_dst),    \
	  lhs.size());							\
  }									\
