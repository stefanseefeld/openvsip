...
inline void                                        \
mat_chol_sol(std::pair<T*,T*> a, int atcols,	   \
...
public:
  ...
  bool decompose(Matrix<T, Block> m) VSIP_NOTHROW
  {
    ...
    if(length_ > 1)
      mat_chol_dec(ext.data(), &idv_[0], length_);
  return true;
}
...
bool
Chold<T>::solve(const_Matrix<T, Block0> b,
		Matrix<T, Block1>       x) VSIP_NOTHROW
{
  ...
  if (length_ > 1) 
  {
    ...
    for(index_type i=0;i<b.size(1);i++) {
      mat_chol_sol(a_ext.data(), a_ext.stride(0),
		   &idv_[0],
