...
template <typename Block,
	  ...
	  typename L = Block_layout<Block>::layout_type>
...
class Distributed_block<B, M, typename B::value_type,
  Layout<B::dim, O, Stride_unit_dense, dense_complex_type> >
...
private:
  ...
  void init(Domain<dim> const& dom)
  {
    ...
    pas::pbuf_create<T, order_type, impl_complex_type>(
      tag_,
      dom,
      map_,
      gdo_handle_,
...
public:
  ...
  value_type get(index_type idx) const VSIP_NOTHROW
  {
    ...
    if (    Type_equal<map_type, Global_map<1> >::value
        ||  vsip::num_processors() == 1
	|| (   Type_equal<map_type, Replicated_map<1> >::value
