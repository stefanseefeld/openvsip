...
/// Description
///   FIR Filter Bank - High Performance 
...
struct t_local_view
{
  ...
  void verify_views(
    ...
    Matrix<T, Block4> expected ATTRIBUTE_UNUSED)
  {
#if PARALLEL_FIRBANK
    // Check that dim-1 is not distributed
    typename Block1::map_type const& map1 ATTRIBUTE_UNUSED = inputs.block().map();
    typename Block2::map_type const& map2 ATTRIBUTE_UNUSED = filters.block().map();
    typename Block3::map_type const& map3 ATTRIBUTE_UNUSED = outputs.block().map();
    typename Block4::map_type const& map4 ATTRIBUTE_UNUSED = outputs.block().map();
