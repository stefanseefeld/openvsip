...
/// Description
///   Benchmark for matrix copy using IPP
...
struct t_mcopy<T, SrcBlock, DstBlock, Impl_copy> : Benchmark_base
{
    ...
    VSIP_IMPL_NOINLINE
  {
    ...
    {
      dda::Data<SrcBlock, dda::in> src_ext(A.block());
      dda::Data<DstBlock, dda::out> dst_ext(Z.block());
    
      t1.start();
      for (index_type l=0; l<loop; ++l)
	ipp_copy(src_ext.ptr(), src_ext.stride(0), src_ext.stride(1),
...
struct t_mcopy<T, SrcBlock, DstBlock, Impl_transpose> : Benchmark_base
{
    ...
    VSIP_IMPL_NOINLINE
  {
	...
	for (index_type l=0; l<loop; ++l)
	  ipp_transpose(
		src_ext.ptr(), src_ext.stride(0), src_ext.stride(1),
	...
	for (index_type l=0; l<loop; ++l)
	  ipp_transpose(
		src_ext.ptr(), src_ext.stride(1), src_ext.stride(0),
		dst_ext.ptr(), dst_ext.stride(0), dst_ext.stride(1),
		N, M);
	t1.stop();
      }
...
struct t_mcopy_par_helper
{
  ...
  typedef typename conditional<SrcDO == 0,
		   row2_type, col2_type>::type
          src_order_type;
  typedef Dense<2, T, src_order_type, map_type> src_block_type;

  typedef typename conditional<DstDO == 0,
		   row2_type, col2_type>::type
          dst_order_type;
