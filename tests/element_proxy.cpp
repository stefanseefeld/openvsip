...
test_proxy(void)
{
  ...
  element_proxy<Dense<1, T> > p(d, 1);
  element_proxy<Dense<1, T> > q(d, 2);
...
test_2d (void)
{
  ...
  element_proxy<Dense<2> > p(d, Index<2>(0, 1));
  test_assert (p == 42);
  ...
  p = element_proxy<Dense<2> >(d, Index<2>(0, 0));
...
test_3d (void)
{
  ...
  element_proxy<Dense<3> > p(d, Index<3>(0, 1, 2));
  test_assert (p == 42);
  ...
  p = element_proxy<Dense<3> >(d, Index<3>(0, 0, 0));
  test_assert(d.get(0,0,0)==42); test_assert(d.get(0,0,1)==42); test_assert(d.get(0,0,2)==42);
...
test_dense_traits()
{
  ...
  OVXX_CT_ASSERT((
    (is_same<typename lvalue_factory_type<Dense<1, T> >::type,
             ref_factory<Dense<1, T> >
    >::value == true) ^ is_split_block<Dense<1, T> >::value));
  OVXX_CT_ASSERT((
    (is_same<typename lvalue_factory_type<Dense<2, T> >::type,
              ref_factory<Dense<2, T> >
    >::value == true) ^ is_split_block<Dense<2, T> >::value));
  OVXX_CT_ASSERT((
    (is_same<typename lvalue_factory_type<Dense<3, T> >::type,
...
main(int argc, char** argv)
{
  ...
  OVXX_CT_ASSERT((
    is_same<lvalue_factory_type<PseudoBlock>::type,
            proxy_factory<PseudoBlock>
    >::value == true));
