...
/// Description
///   Tests for math reductions NOT returning an index.
...
using vsip_csl::maxval;
using vsip_csl::minval;
using vsip_csl::maxmgval;
using vsip_csl::minmgval;
using vsip_csl::maxmgsqval;
using vsip_csl::minmgsqval;
...
test_mgval(Domain<Dim> const& dom, length_type count)
{
  ...
  for (index_type c=0; c<count; ++c)
  {
    ...
    scalar_type lval = maxmgval(store.view);
    test_assert(equal(lval, mag(large)));

    scalar_type sval = minmgval(store.view);
...
test_mgsqval(Domain<Dim> const& dom, length_type count)
{
  ...
  for (index_type c=0; c<count; ++c)
  {
    ...
    scalar_type lval = maxmgsqval(store.view);
    test_assert(equal(lval, magsq(large)));

    scalar_type sval = minmgsqval(store.view);
