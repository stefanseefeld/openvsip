...
template <typename T>
struct t_fft_op<T, interleaved_complex> : Benchmark_base, fft_base<T>
{
  ...
  {
    typedef Layout<1, row1_type, dense, ComplexFmt> LP;
...
template <typename T>
struct t_fft_op<T, split_complex> : Benchmark_base, fft_base<T>
{
  static storage_format_type const ComplexFmt = split_complex;
...
template <typename T>
struct t_fft_op_nd<T, interleaved_complex> : Benchmark_base, fft_base<T>
{
  static storage_format_type const ComplexFmt = interleaved_complex;
...
template <typename T>
struct t_fft_ip<T, interleaved_complex> : Benchmark_base, fft_base<T>
{
  static storage_format_type const ComplexFmt = interleaved_complex;
...
template <typename T>
struct t_fft_ip<T, split_complex> : Benchmark_base, fft_base<T>
{
  static storage_format_type const ComplexFmt = split_complex;
...
test(Loop1P& loop, int what)
{
  ...
  {
#if VSIP_IMPL_PROVIDE_FFT_FLOAT
  case   1: loop(t_fft_op<Cf, interleaved_complex>(false, est, sw)); break;
  case   2: loop(t_fft_ip<Cf, interleaved_complex>(false, est, sw)); break;
  case   4: loop(t_fft_op<Cf, interleaved_complex>(false, est | FFTW_UNALIGNED, sw)); break;
  // case  5: loop(t_fft_op<complex<float>, interleaved_complex>(true)); break;
  // case  6: loop(t_fft_ip<complex<float>, interleaved_complex>(true)); break;

  case  11: loop(t_fft_op<Cf, interleaved_complex>(false, msr, sw)); break;
  case  12: loop(t_fft_ip<Cf, interleaved_complex>(false, msr, sw)); break;
  case  14: loop(t_fft_op<Cf, interleaved_complex>(false, msr | FFTW_UNALIGNED, sw)); break;
  case  15: loop(t_fft_op<Cf, interleaved_complex>(false, msr | FFTW_PRESERVE_INPUT, sw)); break;
  case  16: loop(t_fft_op_nd<Cf, interleaved_complex>(false, msr | FFTW_PRESERVE_INPUT, sw)); break;

  case  21: loop(t_fft_op<Cf, interleaved_complex>(false, pnt, sw)); break;
  case  22: loop(t_fft_ip<Cf, interleaved_complex>(false, pnt, sw)); break;
  case  24: loop(t_fft_op<Cf, interleaved_complex>(false, pnt | FFTW_UNALIGNED, sw)); break;

  case  31: loop(t_fft_op<Cf, interleaved_complex>(false, exh, sw)); break;
  case  32: loop(t_fft_ip<Cf, interleaved_complex>(false, exh, sw)); break;
  case  34: loop(t_fft_op<Cf, interleaved_complex>(false, exh | FFTW_UNALIGNED, sw)); break;

  case  51: loop(t_fft_op<Cf, split_complex>(false, est, sw)); break;
  case  52: loop(t_fft_ip<Cf, split_complex>(false, est, sw)); break;
  case  61: loop(t_fft_op<Cf, split_complex>(false, msr, sw)); break;
  case  62: loop(t_fft_ip<Cf, split_complex>(false, msr, sw)); break;
  case  71: loop(t_fft_op<Cf, split_complex>(false, pnt, sw)); break;
  case  72: loop(t_fft_ip<Cf, split_complex>(false, pnt, sw)); break;
#endif // VSIP_IMPL_PROVIDE_FFT_FLOAT

#if VSIP_IMPL_PROVIDE_FFT_DOUBLE
  case 101: loop(t_fft_op<Cd, interleaved_complex>(false, est, sw)); break;
  case 102: loop(t_fft_ip<Cd, interleaved_complex>(false, est, sw)); break;
  case 104: loop(t_fft_op<Cd, interleaved_complex>(false, est | FFTW_UNALIGNED, sw)); break;
  case 111: loop(t_fft_op<Cd, interleaved_complex>(false, msr, sw)); break;
  case 112: loop(t_fft_ip<Cd, interleaved_complex>(false, msr, sw)); break;
  case 114: loop(t_fft_op<Cd, interleaved_complex>(false, msr | FFTW_UNALIGNED, sw)); break;
  case 121: loop(t_fft_op<Cd, interleaved_complex>(false, pnt, sw)); break;
  case 122: loop(t_fft_ip<Cd, interleaved_complex>(false, pnt, sw)); break;
  case 124: loop(t_fft_op<Cd, interleaved_complex>(false, pnt | FFTW_UNALIGNED, sw)); break;
  case 131: loop(t_fft_op<Cd, interleaved_complex>(false, exh, sw)); break;
  case 132: loop(t_fft_ip<Cd, interleaved_complex>(false, exh, sw)); break;
  case 134: loop(t_fft_op<Cd, interleaved_complex>(false, exh | FFTW_UNALIGNED, sw)); break;
#endif // VSIP_IMPL_PROVIDE_FFT_DOUBLE
