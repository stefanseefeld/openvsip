...
struct Binary_map<expr::Scalar<D, S> const, Block>
{
  typedef typename Block::map_type map_type;
  static map_type const &get(expr::Scalar<D, S> const &, Block const &r)
  { return r.map();}
};
...
public:
  ...
  typedef typename impl::Binary_map<LBlock, RBlock>::map_type map_type;
...
// 	   extent<dim>(lhs_) == extent<dim>(rhs_));
  ...
  { return impl::Binary_map<LBlock, RBlock>::get(lhs_, rhs_);}
...
struct get_block_layout<vsip_csl::expr::Binary<O, A1, A2> const>
  : get_block_layout<typename vsip_csl::expr::Binary<O, A1, A2>::cache_type>
{
};
...
void
assert_local(expr::Binary<O, B1, B2, E> const &, index_type)
{
}
