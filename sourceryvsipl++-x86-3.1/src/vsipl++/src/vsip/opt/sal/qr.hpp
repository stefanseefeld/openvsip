...
inline void						\
mat_qr_dec(T const *a, int tcols_a, T* r, int tcols_r,	\
...
inline void						\
mat_qr_dec(std::pair<T const*,T const*> const& a, int tcols_a,	\
...
inline void							\
mat_qr_rsol(T const *r, int tcols_r, T const *b, T* x, length_type n)	\
{								\
...
inline void							\
mat_qr_rsol(std::pair<T const*,T const*> const& r, int tcols_r,	\
...
inline void							\
mat_qr_rhsol(T const *r, int tcols_r, T const *b, T* x, length_type n)\
{								\
  SALFCN((SAL_T*)r,tcols_r, (SAL_T*)b, (SAL_T*)x, n);		\
}
...
inline void							\
mat_qr_rhsol(std::pair<T const*,T const*> const& r,		\
	     int tcols_r,					\
	     std::pair<T const *,T const*> const& b,		\
	     std::pair<T*,T*> const& x,				\
	     length_type n)					\
...
class Qrd
{
  ...
  typedef Storage<storage_format, T>         cp_storage_type;
...
public:
  ...
  bool decompose(Matrix<T, Block> m) VSIP_NOTHROW
  {
    ...
    dda::Data<t_data_block_type, dda::in> data(data_.block());
    dda::Data<data_block_type, dda::out> r_data(r_data_.block());
    mat_qr_dec(data.ptr(), m_, r_data.ptr(), n_, m_, n_);
    return true;
  }
	      ...
	      Matrix<T, Block1>       x) VSIP_NOTHROW
{
  ...
  if(ps == mat_lside) 
  {
    ...
    generic_prod(tr == mat_trans || tr == mat_herm ? t_data_.block() : data_.block(),
		 b.block(), x.block());
  }
  else
  {
    ...
    generic_prod(b.block(),
		 tr == mat_trans || tr == mat_herm ? t_data_.block() : data_.block(),
		 x.block());
  }
  ...
  VSIP_NOTHROW
{
  ...
  if(tr == mat_ntrans)
  {
    ...
    {
      dda::Data<t_data_block_type, dda::in> b_data(b_int.block());
      dda::Data<t_data_block_type, dda::inout> x_data(x_int.block());
      dda::Data<data_block_type, dda::in> r_data(r_data_.block());

      typename dda::Data<t_data_block_type, dda::in>::ptr_type b_ptr = b_data.ptr();
      typename dda::Data<t_data_block_type, dda::inout>::ptr_type x_ptr = x_data.ptr();
      
      for(length_type i=0;i < b.size(1);i++)
      {
	mat_qr_rsol(r_data.ptr(), n_,
		    dda::impl::offset(b_ptr,i*n_),
		    dda::impl::offset(x_ptr,i*n_),
		    n_);
      }
    }
  ...
  else
  {
...

    {
      dda::Data<t_data_block_type, dda::in> b_data(b_int.block());
      dda::Data<t_data_block_type, dda::inout> x_data(x_int.block());
      dda::Data<data_block_type, dda::in> r_data(rt_data_.block());
  ...
  VSIP_NOTHROW
{
  ...
  generic_prod(t_data_.block(), b.block(), c_int.block());
  ...
  {
    dda::Data<t_data_block_type, dda::inout> x_data(x_int.block());
    dda::Data<t_data_block_type, dda::in> c_data(c_int.block());
    dda::Data<data_block_type, dda::in> r_data(r_data_.block());

    typename dda::Data<t_data_block_type, dda::in>::ptr_type c_ptr = c_data.ptr();
    typename dda::Data<t_data_block_type, dda::inout>::ptr_type x_ptr = x_data.ptr();
