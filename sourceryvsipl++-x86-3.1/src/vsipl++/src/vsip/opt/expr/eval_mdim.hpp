...
public:
  ...
  template <dimension_type Dim,
	    typename       T>
  struct leaf_node<expr::Scalar<Dim, T> const>
  {
    typedef expr::Scalar<1, T> const type;
  };
  ...
  template <dimension_type Dim,
	    typename       T>
  typename transform<expr::Scalar<Dim, T> const>::type
  apply(expr::Scalar<Dim, T> const& block) const
  {
    typedef typename transform<expr::Scalar<Dim, T> const>::type type;
    return type(block.value());
  }
