...
// 1-D unit stride
__global__ void 
sin(float const* in, float* out, size_t length)
{
  ...
  if (idx < length)
    out[idx] = __sinf(in[idx]);
}

__global__ void 
sin(cuComplex const* in, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
// 1-D general stride
__global__ void 
sin(float const* in, ptrdiff_t in_stride, float* out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (tid < length)
    out[out_idx] = __sinf(in[in_idx]);
}

__global__ void 
sin(cuComplex const* in, ptrdiff_t in_stride, cuComplex* out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type      length)
{
  ...
  dev::sin<<<grid, threads>>>(in, out, length);
}
  ...
  length_type      length)
{
  ...
  dev::sin<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in),
  ...
  length_type      length)
{
  ...
  dev::sin<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type                    length)
{
  ...
  dev::sin<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in), in_stride,
...
sin(
  ...
  stride_type      num_rows,
  length_type      num_cols)
{
  dim3 grid, threads;
  distribute_matrix(num_rows, num_cols, grid, threads);

  dev::sin<<<grid, threads>>>(in, row_in_stride, col_in_stride,
