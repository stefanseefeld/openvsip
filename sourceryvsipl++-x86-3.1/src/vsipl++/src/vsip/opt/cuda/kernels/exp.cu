...
// 1-D unit stride
__global__ void
exp(float const *in, float *out, size_t length)
{
...
__global__ void
exp(cuComplex const *in, cuComplex *out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
// 1-D general stride
__global__ void
exp(float const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
__global__ void
exp(cuComplex const *in, ptrdiff_t in_stride, cuComplex *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type     length)
{
  ...
  dev::exp<<<grid, threads>>>(in, out, length);
}
  ...
  length_type                   length)
{
  ...
  dev::exp<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in),
  ...
  length_type     length)
{
  ...
  dev::exp<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type     num_cols)
{
  ...
  dev::exp<<<grid, threads>>>(in, row_in_stride, col_in_stride,
