...
// 1-D unit stride
__global__ void 
log(float const* in, float* out, size_t length)
{
...
__global__ void 
log(cuComplex const* in, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
// 1-D general stride
__global__ void 
log(float const* in, ptrdiff_t in_stride, float* out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (tid < length)
    out[out_idx] = logf(in[in_idx]);
}

__global__ void 
log(cuComplex const* in, ptrdiff_t in_stride, cuComplex* out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type      length)
{
  ...
  dev::log<<<grid, threads>>>(in, out, length);
}
  ...
  length_type                    length)
{
  ...
  dev::log<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in),
  ...
  length_type      length)
{
  ...
  dev::log<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type                    length)
{
  ...
  dev::log<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in), in_stride,
  ...
  length_type      num_cols)
{
  ...
  dev::log<<<grid, threads>>>(in, row_in_stride, col_in_stride,
