...
/// Description
///   CUDA kernel for hyperbolic sine.
...
// 1-D unit stride
__global__ void
sinh(float const *in, float *out, size_t length)
{
  ...
  if (tid < length)
    out[tid] = sinhf(in[tid]);
}

__global__ void
sinh(cuComplex const *in, cuComplex *out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  int const tid = __mul24(blockDim.x, bx) + tx;
  if (tid < length)
    csinh(out[tid], in[tid]);
}

// 1-D general stride
__global__ void
sinh(float const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (tid < length)
    out[out_idx] = sinhf(in[in_idx]);
}

__global__ void
sinh(cuComplex const *in, ptrdiff_t in_stride, cuComplex *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type     length)
{
  ...
  dev::sinh<<<grid, threads>>>(in, out, length);
}
  ...
  length_type                   length)
{
  ...
  dev::sinh<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in),
  ...
  length_type     length)
{
  ...
  dev::sinh<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type     num_cols)
{
  ...
  dev::sinh<<<grid, threads>>>(in, row_in_stride, col_in_stride,
