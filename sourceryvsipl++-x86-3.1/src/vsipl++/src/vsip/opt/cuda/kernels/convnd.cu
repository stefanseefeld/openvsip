...
/// Description
///   CUDA kernels for optimized convolution.
#include <cuComplex.h>
#include <complex>
#include "cmplx.cuh"
#include "util.hpp"
...
// 1-D convolution, shared memory is used to overcome excessive global memory
  ...
  int          output_len)
{
  ...
  for (int i = 0; i <= tx; ++i)
  {
    sum_begin += input_sh[tx - i] * kernel_sh[i];
    sum_end += input_sh[__mul24(kernel_len, 2) - tx + i - 1] *
               kernel_sh[kernel_len - 1 - i];
  }
  ...
  int              output_len)
{
  ...
  for (int i = 0; i <= tx; ++i)
  {
    cmul(temp_begin, input_sh[tx - i], kernel_sh[i]);
    cmul(temp_end, input_sh[__mul24(kernel_len, 2) - tx + i - 1],
                   kernel_sh[kernel_len - 1 - i]);
  ...
  int          shift)
{
  ...
  {
    sum_begin += input_sh[tx + shift - i] * kernel_sh[i];
  ...
  int              shift)
{
  ...
  for (int i = 0; i <= tx + shift; ++i)
  {
    cmul(temp_begin, input_sh[tx + shift - i], kernel_sh[i]);
    cmul(temp_end, input_sh[__mul24(kernel_len, 2) - tx + i - 1 - shift],
                   kernel_sh[kernel_len - 1 - i]);
  ...
  int          shift)
{
  ...
  {
    sum_begin += input_sh[tx + shift - 1 - i] * kernel_sh[i];
  ...
  int              shift)
{
  ...
  for (int i = 0; i <= tx + shift - 1; ++i)
  {
    cmul(temp_begin, input_sh[tx + shift - 1 - i], kernel_sh[i]);
    cmul(temp_end, input_sh[__mul24(kernel_len, 2) - tx + i - shift],
                   kernel_sh[kernel_len - 1 - i]);
  ...
  size_t           ou_len)
{
  ...
  k_convnd_center_full_ss<<<grid, threads>>>(in, kr, out, int(in_len),
                                             int(kr_len), int(ou_len));

  k_convnd_edge_full_ss<<<grid_edge, threads_edge>>>(in, kr, out, int(in_len),
                                                     int(kr_len), int(ou_len));
  ...
  size_t           ou_len)
{
  ...
  k_convnd_center_min_ss<<<grid, threads>>>(in, kr, out, int(in_len),
                                            int(kr_len), int(ou_len));
  ...
  size_t                         ou_len)
{
  ...
  k_convnd_center_min_cc<<<grid, threads>>>(
    reinterpret_cast<cuComplex const*>(in),
    reinterpret_cast<cuComplex const*>(kr),
    reinterpret_cast<cuComplex*>(out), int(in_len),
    int(kr_len), int(ou_len));
  ...
  size_t           shift)
{
  ...
  k_convnd_center_same_odd_ss<<<grid, threads>>>(in, kr, out, int(in_len),
                                                 int(kr_len), int(ou_len),
                                                 int(shift));

  k_convnd_edge_same_odd_ss<<<grid_edge, threads_edge>>>(in, kr, out, int(in_len),
                                                         int(kr_len), int(ou_len),
                                                         int(shift));
  ...
  size_t                         shift)
{
  ...
  k_convnd_center_same_odd_cc<<<grid, threads>>>(
    reinterpret_cast<cuComplex const*>(in),
    reinterpret_cast<cuComplex const*>(kr),
    reinterpret_cast<cuComplex*>(out), int(in_len),
    int(kr_len), int(ou_len), int(shift));

  k_convnd_edge_same_odd_cc<<<grid_edge, threads_edge>>>(
    reinterpret_cast<cuComplex const*>(in),
    reinterpret_cast<cuComplex const*>(kr),
    reinterpret_cast<cuComplex*>(out), int(in_len),
    int(kr_len), int(ou_len), int(shift));
  ...
  size_t           shift)
{
  ...
  k_convnd_center_same_even_ss<<<grid, threads>>>(in, kr, out, int(in_len),
                                                  int(kr_len), int(ou_len),
                                                  int(shift));

  k_convnd_edge_same_even_ss<<<grid_edge, threads_edge>>>(in, kr, out, int(in_len),
                                                          int(kr_len), int(ou_len),
                                                          int(shift));
  ...
  size_t                         shift)
{
  ...
  k_convnd_center_same_even_cc<<<grid, threads>>>(
    reinterpret_cast<cuComplex const*>(in),
    reinterpret_cast<cuComplex const*>(kr),
    reinterpret_cast<cuComplex*>(out), int(in_len),
    int(kr_len), int(ou_len), int(shift));

  k_convnd_edge_same_even_cc<<<grid_edge, threads_edge>>>(
    reinterpret_cast<cuComplex const*>(in),
    reinterpret_cast<cuComplex const*>(kr),
    reinterpret_cast<cuComplex*>(out), int(in_len),
    int(kr_len), int(ou_len), int(shift));
