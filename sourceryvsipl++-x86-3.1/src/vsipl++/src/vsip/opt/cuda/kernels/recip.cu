...
// 1-D unit stride
__global__ void
recip(float const *in, float *out, size_t length)
{
...
__global__ void
recip(cuComplex const *in, cuComplex *out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  int const tid = __mul24(blockDim.x, bx) + tx;
  if (tid < length)
    crecip(out[tid], in[tid]);
}

// 1-D general stride
__global__ void
recip(float const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (tid < length)
    out[out_idx] = 1.0 / in[in_idx];
}

__global__ void
recip(cuComplex const *in, ptrdiff_t in_stride, cuComplex *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type     length)
{
  ...
  dev::recip<<<grid, threads>>>(in, out, length);
}
  ...
  length_type                   length)
{
  ...
  dev::recip<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in),
  ...
  length_type     length)
{
  ...
  dev::recip<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type                   length)
{
  ...
  dev::recip<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in), in_stride,
  ...
  length_type     num_cols)
{
  ...
  dev::recip<<<grid, threads>>>(in, row_in_stride, col_in_stride,
