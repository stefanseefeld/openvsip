...
// 1-D unit stride
__global__ void 
atan2(float const* in1, float const* in2, float* out, size_t length)
{
  ...
  if (idx < length)
    out[idx] = atan2f(in1[idx], in2[idx]);
}
// 1-D general stride
__global__ void 
atan2(float const* in1, ptrdiff_t in1_stride,
      ...
      float* out, ptrdiff_t out_stride, size_t length)
{
  ...
  if (tid < length)
    out[out_idx] = atan2f(in1[in1_idx], in2[in2_idx]);
}
// 2-D general stride
__global__ void 
atan2(float const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
      ...
      size_t num_rows, size_t num_cols)
{
  ...
  int const out_idx = __mul24(tidy, out_row_stride) + __mul24(tidx, out_row_stride);
  ...
  length_type      length)
{
  ...
  dev::atan2<<<grid, threads>>>(in1, in2, out, length);
}
// 1-D general stride
void
atan2(
  ...
  length_type      length)
{
  ...
  dev::atan2<<<grid, threads>>>(in1, in1_stride,
                                in2, in2_stride,
  ...
  length_type      ncols)
{
  ...
  dev::atan2<<<grid, threads>>>(in1, in1_row_stride, in1_col_stride,
