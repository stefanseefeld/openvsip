...
/// Description
///   CUDA kernel for hypotenuse function.
...
// 1-D unit stride
__global__ void 
hypot(float const* in1, float const* in2, float* out, size_t length)
{
  ...
  if (idx < length)
    out[idx] = sqrtf(in1[idx] * in1[idx] + in2[idx] * in2[idx]);
}
// 1-D general stride
__global__ void 
hypot(float const* in1, ptrdiff_t in1_stride,
      ...
      float* out, ptrdiff_t out_stride, size_t length)
{
  ...
  if (tid < length)
    out[out_idx] = sqrtf(in1[in1_idx] * in1[in1_idx] + in2[in2_idx] * in2[in2_idx]);
}
// 2-D general stride
__global__ void 
hypot(float const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
  ...
  length_type      length)
{
  ...
  dev::hypot<<<grid, threads>>>(in1, in2, out, length);
}
// 1-D general stride
void
hypot(
  ...
  length_type      length)
{
  ...
  dev::hypot<<<grid, threads>>>(in1, in1_stride,
                                in2, in2_stride,
  ...
  length_type      ncols)
{
  ...
  dev::hypot<<<grid, threads>>>(in1, in1_row_stride, in1_col_stride,
