...
// 1-D unit stride
__global__ void 
log10(float const* in, float* out, size_t length)
{
...
__global__ void 
log10(cuComplex const* in, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  int const idx = __mul24(blockDim.x, bx) + tx;
  if (idx < length)
    clog10(out[idx], in[idx]);
}

// 1-D general stride
__global__ void 
log10(float const* in, ptrdiff_t in_stride, float* out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (tid < length)
    out[out_idx] = log10f(in[in_idx]);
}

__global__ void 
log10(cuComplex const* in, ptrdiff_t in_stride, cuComplex* out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type      length)
{
  ...
  dev::log10<<<grid, threads>>>(in, out, length);
}
  ...
  length_type                    length)
{
  ...
  dev::log10<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in),
  ...
  length_type      length)
{
  ...
  dev::log10<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type                    length)
{
  ...
  dev::log10<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in), in_stride,
  ...
  length_type      num_cols)
{
  ...
  dev::log10<<<grid, threads>>>(in, row_in_stride, col_in_stride,
