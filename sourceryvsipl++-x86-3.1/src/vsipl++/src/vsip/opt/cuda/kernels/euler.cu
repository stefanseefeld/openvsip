...
/// Description
///   CUDA kernel for euler function.
...
// 1-D unit stride
__global__ void
euler(float const *in, cuComplex *out, size_t length)
{
  ...
  if (tid < length)
    ceuler(out[tid], in[tid]);
}

// 1-D general stride
__global__ void
euler(float const *in, ptrdiff_t in_stride, cuComplex *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type                   length)
{
  ...
  dev::euler<<<grid, threads>>>(in, reinterpret_cast<cuComplex*>(out), length);
}
  ...
  length_type                   length)
{
  ...
  dev::euler<<<grid, threads>>>(in, in_stride, reinterpret_cast<cuComplex*>(out), out_stride, length);
}
  ...
  length_type                   num_cols)
{
  ...
  dev::euler<<<grid, threads>>>(in, row_in_stride, col_in_stride,
