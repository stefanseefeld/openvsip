...
struct Sort_data_helper<ViewT, ViewT, true>
{
  static void exec(ViewT inout)
  {
    ...
    vsip::dda::Data<typename ViewT::block_type, vsip::dda::inout, layout_type>
      raw(inout.block());
    std::sort(raw.ptr(), raw.ptr() + raw.size(0), std::less<typename ViewT::value_type>());
  }
...
struct Sort_data_helper<ViewT, Comp, false>
{
  ...
  {
    typedef vsip::Layout<1, vsip::row1_type,
      vsip::dense> layout_type;
    vsip::dda::Data<typename ViewT::block_type, vsip::dda::inout, layout_type>
      raw(inout.block());
...
///                 sort_functor(ptr(indices(i)), ptr(indices(j))) == false iff i > j
             ...
             FunctorT c)
{
  ...
  vsip::dda::Data<B1, vsip::dda::inout, layout_type> raw(indices.block());
  std::sort(raw.ptr(), raw.ptr() + raw.size(0), comp);
}

/// Return sorted indices in user-supplied vector.
///  :indices:      Overwritten with sorted indices such that
///                 ptr(indices(i)) < ptr(indices(j)) == false iff i > j
///  :data:         data vector, not modified.
