...
{									\
  ...
  return BLASFCN(n, const_cast<T*>(x), incx, const_cast<T*>(y), incy);	\
}
...
VPPFCN(int n,                                                           \
       std::complex<T> const *x, int incx,				\
       std::complex<T> const *y, int incy)				\
{									\
  std::complex<T> ret;  		       				\
  if (incx < 0) x += incx * (n-1);                                      \
  if (incy < 0) y += incy * (n-1);                                      \
...
{									\
  ...
  if (incy < 0) y += incy * (n-1);                                      \
  BLASFCN(&ret, &n,                                                     \
	  const_cast<std::complex<T>*>(x), &incx,                       \
          const_cast<std::complex<T>*>(y), &incy);			\
  return ret;								\
}
#else
# define VSIP_IMPL_BLAS_CDOT(T, VPPFCN, BLASFCN)			\
...
{									\
  ...
  if (incy < 0) y += incy * (n-1);                                      \
  T __complex__ retn =                                                  \
    BLASFCN(&n, const_cast<std::complex<T>*>(x), &incx,                 \
	    const_cast<std::complex<T>*>(y), &incy);			\
...
{									\
  int info;								\
  FCN(&side, &trans, &m, &n, &k, const_cast<T*>(a), &lda,               \
...
inline void								\
potrs(char uplo, int n, int nhrs, T const *a, int lda, T* b, int ldb)	\
{									\
  int info;								\
  FCN(&uplo, &n, &nhrs, const_cast<T*>(a), &lda, b, &ldb, &info);	\
  if (info != 0)							\
  {									\
    char msg[256];							\
...
inline void								\
getrs(char trans, int n, int nhrs, T const *a, int lda, int* ipiv, T* b, int ldb) \
{									\
  int info;								\
  FCN(&trans, &n, &nhrs, const_cast<T*>(a), &lda, ipiv, b, &ldb, &info);\
  if (info != 0)							\
  {									\
    char msg[256];							\
