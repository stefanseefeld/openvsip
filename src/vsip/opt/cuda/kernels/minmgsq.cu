...
/// Description
///   CUDA kernel for minimum squared magnitude.
...
// 1-D unit stride
__global__ void 
minmgsq(float const* in1, float const* in2, float* out, size_t length)
{
  ...
  if (idx < length)
  {
    ...
    out[idx] = fminf(temp1, temp2);
  }
}

__global__ void 
minmgsq(cuComplex const* in1, cuComplex const* in2, float* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
__global__ void 
minmgsq(cuComplex const* in1, float const* in2, float* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
// 1-D general stride
__global__ void 
minmgsq(float const* in1, ptrdiff_t in1_stride,
        ...
        float* out, ptrdiff_t out_stride, size_t length)
{
  ...
  if (tid < length)
  {
    ...
    out[out_idx] = fminf(temp1, temp2);
  }
}

__global__ void 
minmgsq(cuComplex const* in1, ptrdiff_t in1_stride,
...
// 2-D general stride
__global__ void 
minmgsq(float const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
...
__global__ void 
minmgsq(cuComplex const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
  ...
  length_type      length)
{
  ...
  dev::minmgsq<<<grid, threads>>>(in1, in2, out, length);
}
  ...
  length_type                    length)
{
  ...
  dev::minmgsq<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in1),
  ...
  length_type                    length)
{
  ...
  dev::minmgsq<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in2), in1,
                                out, length);
}
// 1-D general stride
void
  ...
  length_type      length)
{
  ...
  dev::minmgsq<<<grid, threads>>>(in1, in1_stride,
                                  in2, in2_stride,
  ...
  length_type                length)
{
  ...
  dev::minmgsq<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in1), in1_stride,
  ...
  length_type                    length)
{
  ...
  dev::minmgsq<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in2), in2_stride,
  ...
  length_type      ncols)
{
  ...
  dev::minmgsq<<<grid, threads>>>(in1, in1_row_stride, in1_col_stride,
  ...
  length_type                    ncols)
{
  ...
  dev::minmgsq<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in2),
