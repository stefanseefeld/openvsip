...
/// Description
///   CUDA kernel for argument function.
...
// 1-D unit stride
__global__ void
arg(cuComplex const *in, float *out, size_t length)
{
  ...
  if (tid < length)
    out[tid] = atan2f(in[tid].y, in[tid].x);
}

// 1-D general stride
__global__ void
arg(cuComplex const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (tid < length)
    out[out_idx] = atan2f(in[in_idx].y, in[in_idx].x);
}
  ...
  length_type                   length)
{
  ...
  dev::arg<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in),
                               out, length);
}
  ...
  length_type                   length)
{
  ...
  dev::arg<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in), in_stride,
