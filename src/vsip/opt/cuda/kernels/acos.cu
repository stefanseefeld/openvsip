...
// 1-D unit stride
__global__ void
acos(float const *in, float *out, size_t length)
{
  ...
  if (tid < length)
    out[tid] = acosf(in[tid]);
}

// 1-D general stride
__global__ void
acos(float const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type     length)
{
  ...
  dev::acos<<<grid, threads>>>(in, out, length);
}
  ...
  length_type     length)
{
  ...
  dev::acos<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type     num_cols)
{
  ...
  dev::acos<<<grid, threads>>>(in, row_in_stride, col_in_stride,
