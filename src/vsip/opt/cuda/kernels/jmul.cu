...
/// Description
///   CUDA kernel for conjugate complex multiply.
...
// 1-D unit stride
__global__ void 
jmul(cuComplex const* in1, cuComplex const* in2, cuComplex* out, size_t length)
{
  ...
  if (idx < length)
    cmulc(out[idx], in1[idx], in2[idx]);
}

__global__ void 
jmul(float const* in1, cuComplex const* in2, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
// 1-D general stride
__global__ void 
jmul(cuComplex const* in1, ptrdiff_t in1_stride,
     ...
     cuComplex* out, ptrdiff_t out_stride, size_t length)
{
  ...
  if (tid < length)
    cmulc(out[out_idx], in1[in1_idx], in2[in2_idx]);
}

__global__ void 
jmul(float const* in1, ptrdiff_t in1_stride,
     cuComplex const* in2, ptrdiff_t in2_stride,
     cuComplex* out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  ...
  if (tid < length)
  {
    out[out_idx].x = in1[in1_idx] * in2[in2_idx].x;
    out[out_idx].y = -in1[in1_idx] * in2[in2_idx].y;
  }
}
// 2-D general stride
__global__ void 
jmul(cuComplex const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
...
__global__ void 
jmul(float const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
     cuComplex const* in2, ptrdiff_t in2_row_stride, ptrdiff_t in2_col_stride,
     cuComplex* out, ptrdiff_t out_row_stride, ptrdiff_t out_col_stride,
     size_t num_rows, size_t num_cols)
{
  ...
  length_type                    length)
{
  ...
  dev::jmul<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in1),
  ...
  length_type                    length)
{
  ...
  dev::jmul<<<grid, threads>>>(in1, reinterpret_cast<cuComplex const*>(in2),
                               reinterpret_cast<cuComplex*>(out), length);
}
// 1-D general stride
void
  ...
  length_type                length)
{
  ...
  dev::jmul<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in1), in1_stride,
  ...
  length_type                    length)
{
  ...
  dev::jmul<<<grid, threads>>>(in1, in1_stride, reinterpret_cast<cuComplex const*>(in2),
  ...
  length_type                    ncols)
{
  ...
  dev::jmul<<<grid, threads>>>(in1, in1_row_stride, in1_col_stride,
                               reinterpret_cast<cuComplex const*>(in2),
                               in2_row_stride, in2_col_stride,
                               reinterpret_cast<cuComplex*>(out),
                               out_row_stride, out_col_stride, nrows, ncols);
