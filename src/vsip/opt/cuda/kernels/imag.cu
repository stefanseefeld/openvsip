...
/// Description
///   CUDA kernel for imaginary part of complex.
...
// 1-D unit stride
__global__ void
imag(cuComplex const *in, float *out, size_t length)
{
...
// 1-D general stride
__global__ void
imag(cuComplex const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type                   length)
{
  ...
  dev::imag<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in),
                               out, length);
}
  ...
  length_type                   length)
{
  ...
  dev::imag<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in), in_stride,
