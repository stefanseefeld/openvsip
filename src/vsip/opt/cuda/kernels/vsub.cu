...
/// Description
///   CUDA kernel for vector subtract.
...
// 1-D unit stride
__global__ void 
vsub(float const* in1, float const* in2, float* out, size_t length)
{
...
__global__ void 
vsub(cuComplex const* in1, cuComplex const* in2, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (idx < length)
  {
    out[idx].x = in1[idx].x - in2[idx].x;
    out[idx].y = in1[idx].y - in2[idx].y;
  }
}

__global__ void
vsub(float const* in1, cuComplex const* in2, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
__global__ void
vsub(cuComplex const* in1, float const* in2, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
// 1-D general stride
__global__ void 
vsub(float const* in1, ptrdiff_t in1_stride,
...
__global__ void 
vsub(cuComplex const* in1, ptrdiff_t in1_stride,
     ...
     cuComplex* out, ptrdiff_t out_stride, size_t length)
{
  ...
  if (tid < length)
  {
    out[out_idx].x = in1[in1_idx].x - in2[in2_idx].x;
    out[out_idx].y = in1[in1_idx].y - in2[in2_idx].y;
  }
}
// 2-D general stride
__global__ void 
vsub(float const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
...
__global__ void 
vsub(cuComplex const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
  ...
  length_type      length)
{
  ...
  dev::vsub<<<grid, threads>>>(in1, in2, out, length);
}
  ...
  length_type                length)
{
  ...
  dev::vsub<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in1),
  ...
  length_type      length)
{
  ...
  dev::vsub<<<grid, threads>>>(in1, in1_stride,
                               in2, in2_stride,
  ...
  length_type                length)
{
  ...
  dev::vsub<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in1), in1_stride,
  ...
  length_type      ncols)
{
  ...
  dev::vsub<<<grid, threads>>>(in1, in1_row_stride, in1_col_stride,
