...
// 1-D unit stride
__global__ void 
vdiv(float const* in1, float const* in2, float* out, size_t length)
{
...
__global__ void 
vdiv(cuComplex const* in1, cuComplex const* in2, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  int const idx = __mul24(blockDim.x, bx) + tx;
  if (idx < length)
    cdiv (out[idx], in1[idx], in2[idx]);
}

__global__ void
vdiv(float const* in1, cuComplex const* in2, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  float den;
  ...
  if (idx < length)
  {
    den = (in2[idx].x * in2[idx].x) + (in2[idx].y * in2[idx].y);
    out[idx].x = in1[idx] * in2[idx].x / den;
    out[idx].y = - in1[idx] * in2[idx].y / den;
  }
}

__global__ void
vdiv(cuComplex const* in1, float const* in2, cuComplex* out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  if (idx < length)
  {
    out[idx].x = in1[idx].x / in2[idx];
    out[idx].y = in1[idx].y / in2[idx];
  }
}
// 1-D general stride
__global__ void 
vdiv(float const* in1, ptrdiff_t in1_stride,
...
__global__ void 
vdiv(cuComplex const* in1, ptrdiff_t in1_stride,
     ...
     cuComplex* out, ptrdiff_t out_stride, size_t length)
{
  ...
  int const out_idx = __mul24(tid, out_stride);
  if (tid < length)
    cdiv (out[out_idx], in1[in1_idx], in2[in2_idx]);
}
// 2-D general stride
__global__ void 
vdiv(float const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
...
__global__ void 
vdiv(cuComplex const* in1, ptrdiff_t in1_row_stride, ptrdiff_t in1_col_stride,
  ...
  length_type      length)
{
  ...
  dev::vdiv<<<grid, threads>>>(in1, in2, out, length);
}
  ...
  length_type                length)
{
  ...
  dev::vdiv<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in1),
  ...
  length_type      length)
{
  ...
  dev::vdiv<<<grid, threads>>>(in1, in1_stride,
                               in2, in2_stride,
  ...
  length_type                length)
{
  ...
  dev::vdiv<<<grid, threads>>>(reinterpret_cast<cuComplex const *>(in1), in1_stride,
  ...
  length_type      ncols)
{
  ...
  dev::vdiv<<<grid, threads>>>(in1, in1_row_stride, in1_col_stride,
