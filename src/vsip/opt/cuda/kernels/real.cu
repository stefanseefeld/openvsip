...
/// Description
///   CUDA kernel for real part of complex.
...
// 1-D unit stride
__global__ void
real(float const *in, float *out, size_t length)
{
...
__global__ void
real(cuComplex const *in, float *out, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
// 1-D general stride
__global__ void
real(float const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
...
__global__ void
real(cuComplex const *in, ptrdiff_t in_stride, float *out, ptrdiff_t out_stride, size_t length)
{
  int const tx = threadIdx.x;
  int const bx = blockIdx.x;
  ...
  length_type     length)
{
  ...
  dev::real<<<grid, threads>>>(in, out, length);
}
  ...
  length_type                   length)
{
  ...
  dev::real<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in),
                               out, length);
}
  ...
  length_type     length)
{
  ...
  dev::real<<<grid, threads>>>(in, in_stride, out, out_stride, length);
}
  ...
  length_type                   length)
{
  ...
  dev::real<<<grid, threads>>>(reinterpret_cast<cuComplex const*>(in),
                               in_stride, out, out_stride, length);
}
  ...
  length_type     num_cols)
{
  ...
  dev::real<<<grid, threads>>>(in, row_in_stride, col_in_stride,
