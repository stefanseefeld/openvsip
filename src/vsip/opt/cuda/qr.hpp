...
#ifndef VSIP_OPT_CUDA_QR_HPP
#define VSIP_OPT_CUDA_QR_HPP
...
template <typename T>
class Qrd : Compile_time_assert<cuda::Traits<T>::valid>
{
...
template <typename Block>
bool
Qrd<T>::decompose(Matrix<T, Block> m) VSIP_NOTHROW
{
  ...
  vsip::dda::Data<data_block_type, dda::inout> data(data_.block());
  ...
  Matrix<T, Block1>       x) VSIP_NOTHROW
{
  ...
  {
    vsip::dda::Data<data_block_type, dda::inout> b_data(b_int.block());
    vsip::dda::Data<data_block_type, dda::inout> a_data(data_.block());
  ...
  Matrix<T, Block1>       x) VSIP_NOTHROW
{
  ...
  {
    dda::Data<data_block_type, dda::in> a_dev(data_.block());
    dda::Data<data_block_type, dda::inout> b_dev(b_int.block());
      
    cuda::trsm('l',		// R appears on [l]eft-side
  ...
  Matrix<T, Block1>       x) VSIP_NOTHROW
{
...

  {
    ...
    cuda::trsm('l',	// R' appears on [l]eft-side
	       'u',	// R is [u]pper-triangular
	       cuda::Traits<T>::trans, // [c]onj/[t]ranspose (conj(R'))
	       'n',	// R is [n]ot unit triangular
	       b_rows, b_cols,
	       alpha,
	       a_dev.ptr(), m_,
  ...
  Matrix<T, Block1>       x) VSIP_NOTHROW
{
  ...
  int blksize   = lapack::mqr_blksize<T>('l', cuda::Traits<T>::trans,
					 c_rows, c_cols, n_);
  ...
  // This is computed on the Host side
  {
    vsip::dda::Data<data_block_type, dda::inout> c_dev(c.block());
    vsip::dda::Data<data_block_type, dda::inout> a_dev(data_.block());

    // 1. compute C = Q'B:     R X = C

    lapack::mqr('l',				// Q' on [l]eft (C = Q' B)
		cuda::Traits<T>::trans,	// [t]ranspose (Q')
		c_rows, c_cols, 
  ...
  // This is computed on the GPU side
  {
    dda::Data<data_block_type, dda::inout> c_dev(c.block());
    dda::Data<data_block_type, dda::in> a_dev(data_.block());
...
{
template <typename T>
struct Evaluator<op::qrd, be::cuda, T>
{
  // The CUDA LU solver supports all BLAS types.
  static bool const ct_valid = impl::cuda::Traits<T>::valid;
  typedef impl::cuda::Qrd<T> backend_type;
};
} // namespace vsip_csl::dispatcher
} // namespace vsip_csl

#endif // VSIP_OPT_CUDA_QR_HPP
