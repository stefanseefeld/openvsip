...
class Svd
{
  ...
  typedef Layout<2, row2_type, dense, split_complex> data_LP;
  typedef Strided<2, T, data_LP>           data_block_type;
  typedef Strided<2, scalar_type, data_LP> rl_data_block_type;

  typedef Layout<2, col2_type, dense, split_complex> cp_data_LP;
  typedef Strided<2, uv_type, cp_data_LP> cp_data_block_type;

  typedef Storage<split_complex, T>       my_storage_type;
		  ...
		  Vector<vsip::scalar_f, Block1> dest) VSIP_NOTHROW
{
  ...
  dda::Data<data_block_type, dda::inout> data_data(data_.block());
  dda::Data<cp_data_block_type, dda::out> u_data(u_.block());
  dda::Data<cp_data_block_type, dda::out> v_data(v_.block());
  dda::Data<rl_data_block_type, dda::in> z_data(zeros_.block());
  dda::Data<Block1, dda::out> d_data(dest.block());
  ...
  bool success =
    sal_mat_svd_dec(const_cast<typename 
      dda::Data<rl_data_block_type, dda::out>::ptr_type>(z_data.ptr()),
		    data_data.ptr(), n_,
		    u_data.ptr(), m_,
		    v_data.ptr(), n_,
		    d_data.ptr(),
...
{
  generic_prod(a.real().block(), b.block(), p.block());
}
...
{
  generic_prod(a.block(), b.block(), p.block());
}
...
void sal_svd_prod_uv(const_Matrix<float,Block0> a,
                     ...
                     Matrix<float,Block2> p)
{
  generic_prod(a.block(), b.real().block(), p.block());
}
