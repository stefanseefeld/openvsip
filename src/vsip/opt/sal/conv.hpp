...
protected:
  ...
  void
  convolve(const_Vector<T, Block0>, Vector<T, Block1>) VSIP_NOTHROW;

  template <typename Block0, typename Block1>
  void
  convolve(const_Matrix<T, Block0>, Matrix<T, Block1>) VSIP_NOTHROW;

  typedef vsip::Layout<dim, typename Row_major<dim>::type, unit_stride, storage_format>
    layout_type;
...
private:
  ...
  const_ptr_type  pcoeff_;

  coeff_view_type sal_coeff_;
  c_data_type     sal_coeff_data_;
  const_ptr_type  sal_pcoeff_;
...
Convolution(ConstViewT<T, Block> filter_coeffs,
  ...
  : coeff_(conv_kernel<coeff_view_type>(Symm, filter_coeffs)),
    ...
    sal_coeff_data_(sal_coeff_.block()),
    sal_pcoeff_(sal_coeff_data_.ptr()),
    kernel_size_(impl::view_domain(coeff_)),
    input_size_(input_size),
    output_size_(impl::conv_output_size(Supp, kernel_size_, input_size, decimation)),
...
void
Convolution<ConstViewT, Symm, Supp, T, n_times, a_hint>::
convolve(const_Vector<T, Block0> in, Vector<T, Block1> out)
VSIP_NOTHROW
{
  ...
  typedef Layout<1, Any_type, any_packing, storage_format> req_LP;
  ...
  typename in_data_type::ptr_type pin    = in_data.ptr();
  typename out_data_type::ptr_type pout   = out_data.ptr();

  stride_type s_in  = in_data.stride(0);
  stride_type s_out = out_data.stride(0);
  stride_type s_coeff = coeff_.block().stride(1, 0);
...
void
Convolution<ConstViewT, Symm, Supp, T, n_times, a_hint>::
convolve(const_Matrix<T, Block0> in, Matrix<T, Block1> out)
VSIP_NOTHROW
{
  ...
  if (Supp == support_full)
  {
    ...
    {
      impl::sal::conv2d_full(pcoeff_, Mr, Mc, coeff_row_stride,
  ...
  else if (Supp == support_same)
  {
	...
	if (n1_r > n0_r && n1_c > n0_c)
	  impl::sal::conv2d(sal_pcoeff_, Mr, Mc, coeff_row_stride,
			    pin,         Nr, Nc, in_row_stride,
