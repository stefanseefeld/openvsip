...
public:
  ...
  processor_iterator processor_begin(index_type sb ATTRIBUTE_UNUSED) const VSIP_NOTHROW
  {
    assert(sb == 0);
...
public:
  length_type impl_num_patches(index_type sb ATTRIBUTE_UNUSED) const VSIP_NOTHROW
  ...
  Domain<Dim2> impl_subblock_domain(index_type sb ATTRIBUTE_UNUSED) const VSIP_NOTHROW
  ...
  template <dimension_type Dim2>
  Domain<Dim2> impl_global_domain(index_type sb ATTRIBUTE_UNUSED, index_type patch ATTRIBUTE_UNUSED)
    const VSIP_NOTHROW
    { assert(sb == 0 && patch == 0); return dom_; }

  template <dimension_type Dim2>
  Domain<Dim2> impl_local_domain (index_type sb ATTRIBUTE_UNUSED, index_type patch ATTRIBUTE_UNUSED)
    const VSIP_NOTHROW
    { assert(sb == 0 && patch == 0); return dom_; }
