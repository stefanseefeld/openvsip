...
template <dimension_type Dim,
	  ...
	  typename       ExtDataT>
void
build_ext_array(typename view_of<Block>::const_type view,
		AppMapT &am,
		ExtDataT **array,
                        ...
                        is_split_block<Block2>::value>
{
  ...
  typedef impl::Persistent_data<src_local_block> src_ext_type;
  typedef impl::Persistent_data<dst_local_block> dst_ext_type;
...
public:
      ...
      dst_ext_ (new dst_ext_type*[dst_.block().map().num_subblocks()])
  {
    ...
    par_chain_assign::build_ext_array<Dim, T2, Block2>(
      src_, src_am_, src_ext_, dda::in);
    par_chain_assign::build_ext_array<Dim, T1, Block1>(
      dst_, dst_am_, dst_ext_, dda::out);
...
Par_assign<Dim, T1, T2, Block1, Block2, Chained_assign>::exec_send_list()
{
...

  {
    ...
    for (; sl_cur != sl_end; ++sl_cur)
    {
      ...
      builder.stitch(ext->non_const_ptr(), (*sl_cur).chain_);
      ext->end();
...
Par_assign<Dim, T1, T2, Block1, Block2, Chained_assign>::exec_recv_list()
{
  ...
  for (; rl_cur != rl_end; ++rl_cur)
  {
    ...
    ext->begin();
    builder.stitch(ext->ptr(), (*rl_cur).chain_);
    ext->end();
